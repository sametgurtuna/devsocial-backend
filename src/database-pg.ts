...
class PostgresDatabase {
  ...
  async updateUserSettings(userId: string, settings: Partial<User["settings"]>): Promise<void> {
  ...
  async sendFriendRequest(fromUserId: string, toUsername: string): Promise<FriendRequest> {
    ...
    console.log(`[DB] Friend request sent: ${fromUserId} -> ${toUser.username}`);
    return request;
  }

  async getIncomingFriendRequests(userId: string): Promise<(FriendRequest & { fromUsername: string })[]> {
    const result = await this.pool.query(
  ...
  async updateActivity(
    ...
    if (existing.rows.length === 0) {
      ...
      await this.pool.query(
        ...
        [id, userId, date, totalSeconds, JSON.stringify(projectsObj), JSON.stringify(languagesObj), now]
      );
    } else {
      ...
      await this.pool.query(
        `UPDATE daily_activities 
         ...
         WHERE id = $5`,
        [newTotal, JSON.stringify(existingProjects), JSON.stringify(existingLanguages), now, id]
      );
    }
  }
  ...
  async getFriendsActivity(userId: string): Promise<FriendActivity[]> {
    ...
    for (const friendId of friendIds) {
      ...
      const lastUpdate = todayActivity ? parseInt(todayActivity.last_update) : 0;
